...
export class YandexMap {
  constructor({
    containerSelector,
    apiKey,
    center = [55.17522909256421, 61.40199161447239],
    zoom = 5,
    lang = "ru_RU",
  ...
  getLayoutContentForBallon(id, info) {
    ...
    const slides = info.images
      .map(
        (image, index) =>
          `<div class="swiperSlider swiper-slide"><img src="${image}" alt="${info.title} - Slide ${index + 1}"></div>`
      )
      .join("");
    ...
    return `<div class="swiper">
            ...
            <div class="markInfo">
              <h3>${title}</h3>
              <div class="markType">${this.iconsPresets[type]}</div>
              <address><p>${street}, ${house}</p></address>
              <div class="markComment">${comment}</div>
            </div>
            <div class="editInfo">
